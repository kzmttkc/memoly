...
test('公開ヘッダとルートは /zure を顔にする', () => {
  ...
  assert.match(legal, /data-legal-fold/)
  assert.match(legal, /送る直前の扱い/)
  assert.doesNotMatch(legal, /if \(!open\) return null/)
  assert.match(drop, /intent/)
  assert.match(below, /無料の点検/)
  assert.match(drop, /ファイルを選ぶ/)
  assert.match(hero, /ずれ1枚の見本/)
  assert.match(hero, /data-drop/)
  assert.match(hero, /data-sample-link/)
  assert.doesNotMatch(hero, /border-dashed/)
