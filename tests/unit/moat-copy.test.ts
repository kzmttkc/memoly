...
test('置き直し失敗で見えている1枚を消さない', () => {
  ...
  assert.match(hero, /aria-busy/)
  assert.match(hero, /onDragEnter/)
...
test('1枚とドロップは一つの枠に入る（Linear House）', () => {
  const hero = read('linear-house/patches/HeroArtifact.tsx')
  const drop = read('app/zure/_components/ZureDrop.tsx')
  const frameAt = hero.indexOf('lh-frame')
  const dropAt = hero.indexOf('就業規則のファイルをここに置く')
  const sampleAt = hero.indexOf('ずれ1枚の見本')
  assert.ok(frameAt > 0 && dropAt > frameAt && sampleAt > dropAt)
