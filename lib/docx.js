...
//   - 「■ 」で始まる  → 節見出し（見出し2相当）
//   - 「──…」の罫線  → 罫線段落（細い文字で出す）
//   - 「# 」で始まる  → 見出し1（2026-09-15・書類を複数枚まとめるとき）
//   - 「\f」だけの行  → 改ページ（同上）
//   - 空行            → 空段落
//   - それ以外        → 本文
...
function para(text, kind) {
  // 改ページ（2026-09-15: 届出の書類を1ファイルに複数枚まとめるため）
  if (kind === 'pagebreak') return '<w:p><w:r><w:br w:type="page"/></w:r></w:p>';
  if (kind === 'h1' && text.startsWith('# ')) text = text.slice(2);
...
function classify(line, index) {
  if (line === '\f') return 'pagebreak';     // 「\f」だけの行＝改ページ
  if (/^# /.test(line)) return 'h1';         // 「# 」で始まる行＝見出し1（書類の表題）
