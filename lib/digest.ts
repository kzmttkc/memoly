...
//   「カード」へ正規化する。カードは診断→起草の連結（就業規則AIの独自価値）を1タップで
...
export interface DigestCard {
  ...
  deadline?: string
  /** 確定度ラベル（prompts方針に整合）。診断・助言は「参考情報」、事実整理寄りは「AIの答え（要確認）」。
   *  「一次回答（要確認）」は旧ラベル（2026-07-11平易化前のキャッシュ済みカード互換のため残置。表示側で新ラベルへ写す）。 */
  ...
  try {
    ...
    for (const c of conflicts) {
      ...
      cards.push({
        ...
        nextAction: 'この判断を最新の法令に照らして見直すか、就業規則AIに相談して整理するとよいでしょう。',
        confidence: '参考情報',
        actionTo: 'chat',
