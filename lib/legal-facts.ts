...
export const LEGAL_FACTS: LegalFact[] = [
  ...
  //   ファクトとして注入し「確定」と答えさせる（未確定な106万円の壁の施行日等とは別扱い）。
  {
    ...
    value:
      '2026年10月1日に施行され、確定しています（「議論中」「予定」ではありません）。事業主に、カスハラ防止の方針の明示・相談体制の整備・行為があったときの対応・被害を受けた従業員への配慮といった雇用管理上の措置が義務づけられます。大企業・中小企業を問わず、すべての事業主が対象です。',
    sourceName:
  ...
  },
  {
    ...
    value:
      '2026年10月1日に施行され、確定しています。求職者（就活生等）に対するセクシュアルハラスメントの防止措置が、カスハラ対策と同じ改正法で事業主に求められます。',
    sourceName:
      '改正労働施策総合推進法（令和7年法律第63号）／施行期日を定める政令（令和8年政令第17号）',
    sourceUrl: 'https://www.mhlw.go.jp/content/11900000/001662576.pdf',
    effectiveDate: '2026-10-01（施行日確定・令和8年政令第17号）',
