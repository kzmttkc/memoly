...
// ----------------------------------------------------------------------------
//   目的（看板「会社を覚える」と実体の一致）:
//     就業規則AIが覚えている会社データ（属性・規程・期限・過去の判断）を、経営者が
...
/**
 * 「まだ就業規則AIに教えていない項目」を導く（未登録の属性＝次に埋めると価値が出る点）。
...
export function buildReportBody(input: ReportInput): string {
  ...
  lines.push('')
  lines.push('就業規則AIが覚えている会社の情報を、1枚で見渡せるように整理しました。数値や期限は登録済みの内容にもとづいています。未登録の項目は「未登録」と表示しています。')
  lines.push('')
  ...
  if (context.decisions.length) {
    lines.push('## 最近の判断の記録')
    lines.push('過去に就業規則AIに相談して決めた内容のうち、新しいものから並べています。')
  ...
  if (hints.length) {
    ...
    lines.push('以下はまだ就業規則AIに登録されていません。登録すると、この1枚まとめや相談の精度が上がります。')
...
//
//   Phase1コンプラ（F6と同一）: 就業規則AIが「社労士監修/AI社労士/法的精度」を名乗らない。
//     このメモは「会社が自分の社労士に渡す自社まとめ」であり、就業規則AIが社労士を名乗る文面を
...
export function buildSharoushiMemoBody(input: SharoushiMemoInput): string {
  ...
  lines.push('')
  lines.push('社労士に相談・引き継ぎをする際にお渡しする、会社側の下準備メモです。就業規則AIが覚えている登録済みの情報を整理しています。未登録の項目は「未登録」と表示しています。相談の往復を減らすための整理資料であり、対応の要否はこのメモでは判断していません。')
  lines.push('')
