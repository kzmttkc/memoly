...
- 逆向きの断定もしない。**書いてあるかどうかだけを見て、書いてある内容が今の法令に
  合っているかは判定しない。**「問題ありません」「適法です」「対応済みです」「これで足ります」と書かない。
  （例: 定年60歳の記載があっても、それが今の法令に合っているかはこの1枚では見ない。
  見たまま「定年の定めの記載があります」とだけ書く。）
...
「ある」と名乗れる条件（written / ops_missing の2つだけが「ある」側。この2語以外を使わない）:
- **引用する文の中に、その item のことを指す言葉が入っていること。**
  入っていなければ written も ops_missing も名乗らず unmentioned にする。
- 近くにある別の条文を引いて「示唆されています」「読み取れます」と書かない。
  例（実際に出た誤り）: 「年次有給休暇は、従業員があらかじめ請求する時季に与える。」を引いて
  『年5日の時季指定』を ops_missing にした。この文にあるのは**従業員が請求する時季**であって、
  **会社が指定する年5日**ではない。引用に「時季指定」も「年5日」も無いので unmentioned。
- 例（同じ誤り）: 賃金の構成・計算方法・支払方法だけの条文を引いて『賃金の締切と支払時期』を
  written にした。引用に締切日も支払日も無いので unmentioned。
- 引用に無いことは what_found にも書かない。書けるのは引用した文から読めることだけ。

評価手順（この順）:
1. 抽出テキストが空、またはほぼ空なら document.extracted_ok=false とし、blocks は空、unread_note に理由を書く。
2. 各 item について、テキストから趣旨または用語を探す。見つかって、**その言葉を含む文を引用できるなら** written。
3. 制度の存在だけ読めて手続・窓口・周知・期限が無いときだけ ops_missing（引用の条件は written と同じ）。
...
2026-10-01 の制度（事実。ここを言い換えない）:
- 改正労働施策総合推進法（令和7年法律第63号）により、顧客等からの著しい迷惑行為（カスタマー
  ハラスメント）への雇用管理上の措置が **措置義務** として2026年10月1日から事業主に課される。
- **「努力義務」と書かない。「義務ではない」と書かない。** 直接の刑事罰は無く、
  是正指導・勧告・企業名公表による、とだけ書いてよい。
- 求職者等に対するセクシュアルハラスメントの措置も、同じ2026年10月1日の施行。
