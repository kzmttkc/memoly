// ============================================================================
// kabau-pack.ts — 就業規則AI → 就業規則AI（sharoushi-agent.com）カスハラ実務パック導線の SSOT
...
//
//   文言は 就業規則AI側 site/kasuhara-*-guide.html の .pack-cta 既存文をそのまま使う
...
//   就業規則AI側は utm_source=banto / utm_campaign=kabau_set で着地を識別する。
//
//   このファイルは Node の単体テスト（tests/unit/*.test.ts）から直接 import されるため、
//   '@/...' エイリアスや JSON import を使わない（相対 import も不要な純粋定数＋関数）。
// ============================================================================

/** 就業規則AI 実務パックLP（utm 付き）。WORK_ORDERS.md #3 で確定したURLをそのまま使う。 */
...
/**
 * 就業規則AI側 site/kasuhara-shugyokisoku-kitei-guide.html の .pack-cta（上段）と同一文。
