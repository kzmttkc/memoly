// ============================================================================
// lib/plans.ts — Kabau（カバウ） プラン定義の単一の正本（SSOT）
// ----------------------------------------------------------------------------
// 目的:
...
export interface PlanDef {
  ...
  displayName: string
  /**
   * 月額（円・**利用者が実際に支払う総額**）。free は 0。表示は `¥${monthlyJpy.toLocaleString()}`。
   *
   * 2026-07-30 法務監査でここを「税抜想定」から改めた際の実装要件は不変:
   * **Stripe 側で税を上乗せする実装を入れてはならない**。入れると表示額と請求額が
   * 食い違う（景表法の有利誤認）。この額がそのまま請求額になる。
   *
   * 2026-08-20 正典整合（business-facts.md 2026-07-30 Takeshi確定）: ただし当社は消費税の
   * 免税事業者であり、この額に消費税が含まれると説明するのは誤り。UI・ドキュメントとも
   * 消費税区分を名乗らず、単に支払総額として扱う。
   */
  monthlyJpy: number
  /** 年額（円・支払総額。消費税区分は名乗らない＝上の monthlyJpy と同じ扱い）。年額を提供するプランのみ。提供しないプランは null。 */
   ...
   * Kabauの価値は「自社の規程を覚えていること」なので、**価値そのものを上限にする**。
...
// 「Kabauは現在 無料モニター中で、キー投入後に課金を有効化する」ためのフラグ。
