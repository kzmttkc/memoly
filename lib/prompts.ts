...
//   背景: チャットが「就業規則AI自身の機能・料金・複数顧問先の扱い」を知らず、汎用・回避的に
...
【就業規則AIという製品そのものについての知識（利用者から就業規則AIの機能・料金・仕様を尋ねられたときに使う。ここに書かれていないことは推測で答えず「現在は対応していません」または「就業規則AIでは確認できかねます」と正直に答える）】
■ 就業規則AIでできること（実装済みの機能）
...
//   断定はせず「最新改正を反映していない可能性」を就業規則AIが指摘できるよう注入する。
...
export function buildCompanySystemPrompt(
  ...
  // 【過去の自社判断】ブロック: 「前回はこう決めました」と参照できるようにする（就業規則AIの差別化の核）。
  ...
  //   断定せず「確認対象」として就業規則AIが自然に指摘できるよう注入する（Phase1コンプラ）。
...
${difyContext.answer}`
  ...
  // 就業規則AI自身の機能・料金・複数顧問先の扱いを system に注入（P06是正・LP/課金UIと一致）。
...
- 就業規則AIの機能・料金・プラン・複数顧問先の管理・記憶の仕組みについて聞かれたら、上の【就業規則AIという製品そのものについての知識】に沿って具体的に答えてください（LP・料金表と一致します）。とくに「複数の会社（顧問先）を管理できるか」「記憶は会社ごとに分かれるか」には、士業プランで実現できることを回避せず案内してください。ここに載っていない機能を聞かれたら「現在は対応していません」と正直に伝え、あるかのように装わないでください。
...
//   このメモは就業規則AIが社労士を名乗るものではなく、会社側が登録データを整理した資料である。
...
入力として、就業規則AIが登録済みデータから組み立てた「まとめ本文」を受け取ります。本文そのものは別で表示されるため、あなたが本文を書き写す必要はありません。
 ...
 *   （コードで組んだ骨格）にしか無く、body は LLM を一切通さない。LLM に返させるのは
 *   前書きとむすびの短文だけで、route がコード側で `前書き＋body＋むすび` を連結する。
 *   読み手は「会社が相談する社労士（プロ）」。就業規則AIが社労士を名乗る文面は書かせない。
 */
...
【文体・コンプライアンス】
- 敬体（です・ます）。「社労士監修」「AI社労士」「法的精度」等は使わない（就業規則AIが社労士を名乗らない）。読み手を「あなた」と繰り返し呼ばない。強調記号（* や #）を使わない。`
}
