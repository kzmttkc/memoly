...
// ============================================================================
// Kabauという製品そのものについての確定仕様（2026-07-24 Loop3ドッグフーディング是正）
//   背景: チャットが「Kabau自身の機能・料金・複数顧問先の扱い」を知らず、汎用・回避的に
...
【Kabauという製品そのものについての知識（利用者からKabauの機能・料金・仕様を尋ねられたときに使う。ここに書かれていないことは推測で答えず「現在は対応していません」または「Kabauでは確認できかねます」と正直に答える）】
■ Kabauでできること（実装済みの機能）
- 会社の記憶：就業規則などの規程の要点・過去の判断・対象者を覚えます。PDF・Word（.docx）・テキストのファイル取込に対応しています（スキャン画像など本文が取れないページは未読として残します）。
...
■ 料金プラン（有料プランはご自身の操作で申し込んだときのみ課金されます。登録しただけで自動的に料金が発生することはありません）
...
- 無料プラン・Entry・Standard は1社のみの管理です。
...
//   断定はせず「最新改正を反映していない可能性」をKabauが指摘できるよう注入する。
...
export function buildCompanySystemPrompt(
  ...
  // 【過去の自社判断】ブロック: 「前回はこう決めました」と参照できるようにする（Kabauの差別化の核）。
  ...
  //   断定せず「確認対象」としてKabauが自然に指摘できるよう注入する（Phase1コンプラ）。
...
${difyContext.answer}`
  ...
  // Kabau自身の機能・料金・複数顧問先の扱いを system に注入（P06是正・LP/課金UIと一致）。
...
- Kabauの機能・料金・プラン・複数顧問先の管理・記憶の仕組みについて聞かれたら、上の【Kabauという製品そのものについての知識】に沿って具体的に答えてください（LP・料金表と一致します）。とくに「複数の会社（顧問先）を管理できるか」「記憶は会社ごとに分かれるか」には、士業プランで実現できることを回避せず案内してください。ここに載っていない機能を聞かれたら「現在は対応していません」と正直に伝え、あるかのように装わないでください。
...
- 就業規則のファイル（PDF・Word・テキスト）は、書類ページから取り込めます。スキャン画像で本文が取れない場合は、そのページを貼り付けて補うよう案内してください。取込済みの原文がある相談では、一般論より先に自社の条文を引用します。
...
- 回答が長くなるとき（おおむね5行／300字を超えるとき）は、本文の冒頭に「【要点】」として結論を1〜2行で先に示してから、詳細を続けてください。読み手が全文をスクロールしなくても要旨をつかめるようにします。短い回答に無理に要点を付ける必要はありません（かえって冗長になります）。確定度ラベルは要点の前に置いてください（例：先頭に確定度ラベル→【要点】→詳細）。
${NO_MARKDOWN_RULE}
...
- 回答の最後に【出典】を3行で付けてください（本文中の【根拠】と混ぜない）。形式は次のとおり：
  - 自社: 取込済み規程名（登録日があれば添える）。無ければ「この会社の規程は未登録」。
  - 法令: 確定ファクトのラベルと出典名と施行日。無ければ「この質問に当てはまる確定ファクトは未選択」。
  - 未登録: まだ無い規程名。無いときはこの行を省略。
  一般論と自社決定を同じ行に書かないでください。`
}
...
//   このメモはKabauが社労士を名乗るものではなく、会社側が登録データを整理した資料である。
...
入力として、Kabauが登録済みデータから組み立てた「まとめ本文」を受け取ります。本文そのものは別で表示されるため、あなたが本文を書き写す必要はありません。
 ...
 *   （コードで組んだ骨格）にしか無く、body は LLM を一切通さない。LLM に返させるのは
 *   前書きとむすびの短文だけで、route がコード側で `前書き＋body＋むすび` を連結する。
 *   読み手は「会社が相談する社労士（プロ）」。Kabauが社労士を名乗る文面は書かせない。
 */
...
【文体・コンプライアンス】
- 敬体（です・ます）。「社労士監修」「AI社労士」「法的精度」等は使わない（Kabauが社労士を名乗らない）。読み手を「あなた」と繰り返し呼ばない。強調記号（* や #）を使わない。`
}
