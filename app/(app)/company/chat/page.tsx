...
interface Message {
  ...
  content: string
  isTyping?: boolean
  // 記憶想起の可視化: この回答を作るのに就業規則AIが参照した「自社の記憶」の非PIIサマリ。
  //   サーバの X-Recalled-Memory ヘッダ由来。assistant メッセージにだけ付く。
  recalled?: RecalledMemory
}

// 「この相談で参照した自社の記憶」パネル。就業規則AIの核＝“覚えていて踏まえて答える”を、
...
//   「就業規則AIはこの前提で答えます」の安心感＝混線しない・毎回説明し直さなくてよい、の可視化。
...
//   Phase1コンプラ: 就業規則AIが専門家を仲介する表現は使わない（メモを作って自分で相談に行く導線）。
const UNCERTAIN_RE =
  /断定できません|判断が分かれ|個別の事情|個別の判断|専門家|社労士[にへ]|労働基準監督署に確認|確認をおすすめ/

// P01/P07(2026-07-24): 回答が「自社の規程・情報が未登録なので分からない」と述べたとき、
//   その場から規程を覚えさせる入口（/company/rules）を出す。就業規則AIの看板「会社を覚える」の
//   価値が、まさに情報が無い瞬間に一歩で回収できるようにする。system プロンプトが未登録時に
//   使う定型（「登録されていません」「就業規則AIに伝えて登録」「取込済みの範囲では見当たりません」等）
//   に一致する語で検出する。
const TEACH_RULES_RE =
  /登録されていません|登録されていない|未登録|取込済みの範囲では見当たりません|就業規則AIに(?:伝えて|覚え)|覚えさせ|登録していただく|登録すると次回/

// I2(2026-07-24): 課金/解約/退会など「就業規則AIの労務スコープ外」の質問を検知する。
...
const BILLING_FAQ_TEXT =
  'ご料金や解約についてのご質問ですね。就業規則AIの労務相談とは別のご案内になりますが、こちらでお答えします。\n\n' +
  '就業規則AIは無料プランでもご利用いただけます。有料プランへの切り替えは、会社ページの「プランの管理」からご自身で申し込んだときのみ課金され、登録しただけで自動的に料金が発生することはありません。\n\n' +
  BILLING_PLAN_LINE +
...
function CompanyChat() {
  ...
  // 判断採取フック（TOP5 #1）: 就業規則AIが「この方針を記録しますか？」と能動提案する状態。
    ...
    async (text?: string) => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          // G-h: 「接続に失敗しました」で突き放さない。就業規則AI側の不調と認め、次の一手を添える。
          setMessages(prev => [
            ...prev,
            {
              role: 'assistant',
              content:
                '申し訳ありません。就業規則AIがうまく答えを返せませんでした。少し時間をおいて、同じ内容をもう一度送っていただけますか。ここまでのやり取りは画面に残っています。',
            },
          ])
          return
        }
        ...
        // --- 判断採取フック（TOP5 #1）: 就業規則AI側から能動的に「記録しますか？」を提案 ---
  ...
  return (
    ...
    <div className="mx-auto flex max-w-3xl flex-col" style={{ height: 'calc(100dvh - 8rem)' }}>
      ...
      {/* 自社プロファイル要約ミニバー（G-c）: 「就業規則AIはこの前提で答えます」を1行で常時表示。
                ...
                <div className="mt-1.5 space-y-1.5 pl-1">
                  ...
                  {showUncertainNext && (
                    <p className="text-[11px] leading-relaxed text-neutral-500">
                      AIだけでは判断しきれない内容です。就業規則AIが覚えている自社情報を、社労士に相談するときの下準備メモに整理できます。
                    </p>
                  )}
      ...
      {/* 判断採取フック（TOP5 #1）: 就業規則AIが能動的に「この方針を記録しますか？」と促す。
        ...
        <div className="mt-3 flex flex-col gap-2 rounded-xl border border-brand-200 bg-brand-50 px-4 py-3 sm:flex-row sm:items-center">
          <div className="flex min-w-0 items-start gap-2">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-neutral-600">
                {decisionPrompt.reason}。担当者が代わっても就業規則AIが覚えています。
              </p>
            </div>
          </div>
