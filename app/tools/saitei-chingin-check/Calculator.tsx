...
function SignupCta({ result }: { result: Result }) {
  ...
  return (
    <ToolSignupCta
      ...
      body={
        highPain
          ? '最低賃金は毎年10月ごろに改定され、都道府県ごとに額も発効日も違います。Kabauに無料登録して会社を作ると、いま画面に出ている点検結果がそのまま「会社の記憶」に保存されます。都道府県や社員ごとの時給を覚えさせれば、改定のたびに一から調べ直さずに、影響が出そうな人から一緒に確認できます。'
          : 'Kabauに無料登録して会社を作ると、いま画面に出ている点検結果がそのまま「会社の記憶」に保存されます。Kabauは勤務地や時給の設定を覚えるので、毎年の最低賃金改定への確認を、前提を入れ直さずに続けられます。'
      }
