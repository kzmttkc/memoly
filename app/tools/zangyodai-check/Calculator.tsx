...
function SaveResultCta({ result, input }: { result: ZangyodaiResult; input: ZangyodaiInput }) {
  ...
  return (
    <ToolSignupCta
      ...
      body={
        highPain
          ? '目安を下回っている可能性が見つかったら、来月も同じ点検が要ります。Kabauに無料登録して会社を作ると、いま画面に出ている点検結果がそのまま「会社の記憶」に保存され、自社の手当や所定労働時間を覚えたKabauと、次からの確認を一緒に整理できます。'
          : 'Kabauに無料登録して会社を作ると、いま画面に出ている点検結果がそのまま「会社の記憶」に保存されます。Kabauは自社の手当や所定労働時間を覚えるので、次からは前提を入れ直さずに点検の続きを相談できます。'
      }
