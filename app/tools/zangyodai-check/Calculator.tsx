    ...
    <div className="space-y-6">
      ...
      <Card className="flex flex-col">
        {/* 2026-07-28 CTO修正（L1監査#6）: 専門用語の解説がフォームより下（ページ下部の
            ...
            2026-07-30 UX監査 #7: この段落だけで 320px 幅では約8行あり、最初の入力欄を
            フォールドの外へ押し出していた。**モバイルのみ** order でフォームの下へ回す
...
function SaveResultCta({ result, input }: { result: ZangyodaiResult; input: ZangyodaiInput }) {
  ...
  return (
    <ToolSignupCta
      ...
      body={
        highPain
          ? '目安を下回っている可能性が見つかったら、来月も同じ点検が要ります。就業規則AIに無料登録して会社を作ると、いま画面に出ている点検結果がそのまま「会社の記憶」に保存され、自社の手当や所定労働時間を覚えた就業規則AIと、次からの確認を一緒に整理できます。'
          : '就業規則AIに無料登録して会社を作ると、いま画面に出ている点検結果がそのまま「会社の記憶」に保存されます。就業規則AIは自社の手当や所定労働時間を覚えるので、次からは前提を入れ直さずに点検の続きを相談できます。'
      }
