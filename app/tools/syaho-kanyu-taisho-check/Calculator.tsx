    ...
    <div className="space-y-6">
      ...
      {result && (
        <Card ref={resultRef} role="status" aria-live="polite" className="scroll-mt-4">
          ...
          {/* ===== 就業規則AI 登録CTA（結果末尾に1本・高痛点/低痛点で出し分け） ===== */}
...
function SignupCta({ result, input }: { result: SyahoKanyuResult; input: SyahoKanyuInput | null }) {
  ...
  return (
    <ToolSignupCta
      ...
      body={
        highPain
          ? '加入対象になりそうな人が1人見つかったら、他のパート従業員にも同じ確認が要ります。就業規則AIに無料登録して会社を作ると、いま画面に出ている点検結果がそのまま「会社の記憶」に保存されます。自社の従業員数や勤務形態も覚えさせれば、賃金要件の撤廃（2026年10月予定）など制度が変わったときも、条件を入れ直さずに同じ点検を続けられます。'
          : '就業規則AIに無料登録して会社を作ると、いま画面に出ている点検結果がそのまま「会社の記憶」に保存されます。就業規則AIは自社の従業員数や勤務形態を覚えるので、賃金要件の撤廃（2026年10月予定）や企業規模要件の縮小で前提が変わったときも、入れ直さずに同じ点検を続けられます。'
      }
