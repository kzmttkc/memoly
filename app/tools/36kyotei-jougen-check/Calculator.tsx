...
// CTA = Kabau 無料登録。既存signupのUTM受け皿(app/(auth)/signup)に合わせる。
const SIGNUP_HREF =
    ...
    <div className="space-y-6">
        ...
        <form onSubmit={handleCheck} className="order-1 space-y-5 sm:order-2" noValidate>
          ...
          <div className="space-y-5 rounded-2xl border border-neutral-200 bg-neutral-50/60 p-4 sm:p-5">
            ...
            <div>
              ...
              />
              <p className="mt-1 text-xs text-neutral-500">
                連続する2〜6か月の平均（時間外+休日）のうち、最も大きい値です。いずれの平均も月80時間以内の目安に照らします。分からなければ空欄で構いません。Kabauに登録すると、勤怠から自動で計算します。
              </p>
            </div>
      ...
      {result && (
        <Card ref={resultRef} role="status" aria-live="polite" className="scroll-mt-4">
          ...
          {/* ===== Kabau 登録CTA（結果末尾に1本のみ・痛点別に出し分け） ===== */}
...
// 結果末尾のKabau登録CTA（C08: zangyodai方式の横展開）。
...
function SignupCta({ result }: { result: Result }) {
  ...
  return (
    <ToolSignupCta
      ...
      body={
        highPain
          ? '確認が要りそうな箇所が見つかったら、来月も再来月も同じ点検が要ります。Kabauに無料登録して会社を作ると、いま画面に出ている点検結果がそのまま「会社の記憶」に保存されます。自社の36協定の内容や勤務形態も覚えさせれば、月45時間や単月100時間などの上限の点検を、次からは数字の前提を入れ直さずに毎月続けられます。'
          : 'Kabauに無料登録して会社を作ると、いま画面に出ている点検結果がそのまま「会社の記憶」に保存されます。Kabauは自社の36協定の内容や勤務形態を覚えるので、毎月の同じ点検を、前提を入れ直さずに続けられます。'
      }
