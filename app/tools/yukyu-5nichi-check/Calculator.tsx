...
function SignupCta({ result }: { result: Result }) {
  ...
  return (
    <ToolSignupCta
      ...
      body={
        highPain
          ? 'あと何日足りないかは、社員ごとに基準日も期限も違います。就業規則AIに無料登録して会社を作ると、いま画面に出ている点検結果がそのまま「会社の記憶」に保存されます。社員ごとの基準日と取得状況も覚えさせれば、次からは1人ずつ入れ直さずに、足りていなさそうな人から一緒に確認できます。'
          : '就業規則AIに無料登録して会社を作ると、いま画面に出ている点検結果がそのまま「会社の記憶」に保存されます。就業規則AIは社員ごとの基準日や付与ルールを覚えるので、毎年の同じ点検を、前提を入れ直さずに続けられます。'
      }
