...
//   3つ選ぶ → 足す条文と10月1日までの順番が**押したその場で全文**出る → 残すならメール1つで Word。
...
export default function PageDocBox({ slug }: { slug: string }) {
  ...
  function reveal() {
    ...
    setOut(text)
    setMsg('この画面に全文が出ています。印刷もできます。残すならメールを書いてください。')
  ...
  async function send() {
    ...
    setState('sending')
    track('page_doc_submit', { doc: 'kitei', source: SOURCE, slug })
    try {
      ...
      setState('done')
      setMsg('画面の下に Word の内容を出しています。同じものを入力された宛先へ送りました。')
      track('lead_captured', { source: SOURCE, doc: 'kitei', slug })
    } catch (e) {
      setState('error')
    ...
    <Card className="mt-7 border-[#165E83] p-5 sm:p-6">
      ...
      {out && (
        <>
          ...
          {state !== 'done' && (
            <div className="mt-4">
              <label htmlFor={`pd-email-${slug}`} className="block text-sm font-bold text-neutral-900">この条文と順番を Word で残す（任意）</label>
              <div className="mt-2 flex flex-col gap-2 sm:flex-row">
                ...
                <button type="button" onClick={send} disabled={state === 'sending'} className={buttonClass({ variant: 'primary', size: 'lg' })}>
                  {state === 'sending' ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Word ファイル（.docx）をメールで送る'}
                </button>
              </div>
