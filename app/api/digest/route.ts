...
// 送信していた。現在このリポジトリは「番頭」SaaSであり、送信元 DIGEST_FROM_EMAIL
// は 4 経路共通の番頭アドレスに切り替わっている。放置すると:
//   - Memoryを引用したMemoly名義のメールが番頭ドメインから実ユーザーへ届く
...
// 空エンドポイントにして構造的に無害化する。番頭の正規ダイジェストは
  ...
  return NextResponse.json(
    {
      error: 'Gone',
      detail:
        '/api/digest は廃止されました。番頭のメール配信は /api/company/* を使用してください。',
    },
    { status: 410 }
  )
}
