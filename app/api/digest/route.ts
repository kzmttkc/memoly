...
// 送信していた。現在このリポジトリは「Kabau」SaaSであり、送信元 DIGEST_FROM_EMAIL
// は 4 経路共通のKabauアドレスに切り替わっている。放置すると:
//   - Memoryを引用したMemoly名義のメールがKabauドメインから実ユーザーへ届く
...
// 空エンドポイントにして構造的に無害化する。Kabauの正規ダイジェストは
  ...
  return NextResponse.json(
    {
      error: 'Gone',
      detail:
        '/api/digest は廃止されました。Kabauのメール配信は /api/company/* を使用してください。',
    },
    { status: 410 }
  )
}
