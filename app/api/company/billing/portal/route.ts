...
//     つまり省略すると、Kabauの顧客がポータルで「UITruth Agency」へ自分で乗り換えられる。
//     Kabauの席数トリガ(trg_company_seat_limit)も課金UIも全部迂回する経路になる。
//
//     そこでKabau専用の設定 BANTO_PORTAL_CONFIGURATION を作った（2026-07-30）:
//       キャンセル       … 有効・**請求期間の終了時**（/tokushoho の「末日まで利用可」と一致）
//       決済手段の更新   … 有効（滞納からの自力復帰に必須。既定は無効だった）
//       請求書の履歴     … 有効（法人顧客の経理用）
//       プラン切替       … **無効**（上記の理由。プラン変更はKabau自身の課金UIで行う）
...
/** Kabau専用のカスタマーポータル設定（KIZUNA Creation アカウント）。上のコメント参照。 */
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    //   Stripe ダッシュボード側で誰でも変えられ、変えてもKabauのコードは何も
